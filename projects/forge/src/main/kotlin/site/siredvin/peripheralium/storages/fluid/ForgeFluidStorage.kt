    ...
    override fun takeFluid(predicate: Predicate<FluidStack>, limit: Int): FluidStack {
        ...
        for (i in 0..handler.tanks) {
            ...
            if (predicate.test(storedFluid.toVanilla()) && (forgeStack.isEmpty || storedFluid.isFluidEqual(forgeStack))) {
                val extractedStack = handler.drain(storedFluid.copyWithCount(minOf(storedFluid.amount, realLimit)), IFluidHandler.FluidAction.EXECUTE)
